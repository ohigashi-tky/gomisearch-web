...
- `category`: `普通ごみ` / `不燃ごみ` / `粗大ごみ` / `資源` / `容器包装プラスチック` / `古紙・衣類` / `危険ごみ` / `収集不可`
  - 自治体ごとの呼称 (可燃ごみ・燃やすごみ・燃やせないごみ 等) は取り込み時にこの区分へ寄せる。
    `不燃ごみ` は燃えないごみ (陶器・ガラス・小型金属等)、`危険ごみ` はスプレー缶・ライター・
    電池・蛍光管等の発火性・有害ごみに対応する
